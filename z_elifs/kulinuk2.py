def elif12(number, element):
    ...
    if element == 1:
        ...
        l1 = 2 * p * number
        s1 = p * (number ** 2)
        return('D - ', d1, 'L - ', '{:2.3f}'.format(l1), 'S - ', s1, element)
    elif element == 2:
        r1 = number / 2
        l1 = 2 * p * r1
        s1 = p * (r1 ** 2)
        return('R - ', '{:2.3f}'.format(r1), 'L - ', '{:2.3f}'.format(l1), 'S - ', '{:2.3f}'.format(s1), element)
    elif element == 3:
        r1 = number / 2 * p
        d1 = 2 * r1
        s1 = p * (r1 ** 2)
        return('D -', '{:2.3f}'.format(d1), 'R - ', '{:2.3f}'.format(r1), 'S - ', '{:2.3f}'.format(s1), element)
    elif element == 4:
        r1 = (number / p) ** 0.5
        d1 = 2 * r1
        l1 = 2 * p * r1
        return('D - ', '{:2.3f}'.format(d1), 'L -', '{:2.3f}'.format(l1), 'R - ', '{:2.3f}'.format(r1), element)
...
def elif13(number, element):
    if element == 1:
        ...
        h = c / 2
        s = c * (h / 2)
        return('C- ', '{:2.3f}'.format(c), 'h - ', '{:2.3f}'.format(h), 's -', '{:2.3f}'.format(s), element)
    elif element == 2:
        a = number / (2 ** 0.5)
        h = number / 2
        s = number * (h / 2)
        return('A - ', '{:2.3f}'.format(a), 'h - ', '{:2.3f}'.format(h), 'S -', '{:2.3f}'.format(s), element)
    elif element == 3:
        c = number * 2
        s = c * element / 2
        a = c / (2 ** 0.5)
        return('A - ', '{:2.3f}'.format(a), 'S - ', '{:2.3f}'.format(s), 'C -', '{:2.3f}'.format(c), element)
    elif element == 4:
        c = number / 4
        h = c / 2
        a = c / (2 ** 0.5)
        return('A - ', '{:2.3f}'.format(a), 'h - ', '{:2.3f}'.format(h), 'C -', '{:2.3f}'.format(c), element)
...
def elif14(number, element):
    if element == 1:
        ...
        r2 = r1 * 2
        s = (number ** 2) * (3 ** 0.5) / 4
        return('R1 - ', r1, 'R2 - ', r2, 'S - ', s, element)
    elif element == 2:
        a = number * 6 / 3**0.5
        r2 = 2 * number
        s = (a ** 2) * (3 ** 0.5) / 4
        return('A - ', a, 'R2 - ', r2, 'S - ', s, element)
    elif element == 3:
        r1 = number / 2
        a = r1 * 6 / 3**0.5
        s = (a ** 2) * (3 ** 0.5) / 4
        return('R1 - ', r1, 'A - ', a, 'S - ', s, element)
    elif element == 4:
        a = (number * 4 / (3 ** 0.5)) ** 0.5
        r1 = a * (3 ** 0.5) / 6
        r2 = r1 * 2
        return('A - ', a, 'R1 - ', r1, 'R2 - ', r2, element)
    elif 1 < element < 4 and number < 0:
        return('0 ввод числа')
...
def elif16(years_old):
    if 20 <= years_old <= 69:
        if (years_old % 10 == 1) and (years_old != 11):
            m = 'год'
        elif (years_old % 10 > 1) and (years_old % 10 < 5) and (years_old != 12) and (years_old != 13) and (
                years_old != 14):
            m = 'года'
        else:
            m = 'лет'
...
def elif17(int_number):
    if 10 <= int_number <= 40:
        if (int_number % 10 == 1) and (int_number != 11):
            m = 'учебное задание'
        elif (int_number % 10 > 1) and (int_number % 10 < 5) and (int_number != 12) and (int_number != 13) and (
                int_number != 14):
            m = 'учебных задания'
        else:
            m = 'учебных заданий'
...
def elif19(year):
    c = ['Зеленого', 'Красного', 'Желтого', 'Белого', 'Черного']
    c1 = ['Зеленой', 'Красной', "Желтой", 'Белой', 'Черной']
    a = ['крысы', 'быка', 'тигра', 'кролика', 'дракона', 'змеи', 'коня', 'овцы', 'обезьяны', 'курицы', 'собаки',
         'свиньи']
    year = abs(1984 - year)
    while year > 60:
        year = year - 60
