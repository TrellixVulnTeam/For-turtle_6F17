...
def elif_r6(number, length):
    ...
    Для уроков (составитель тестов) - дециметр, 2 - километр, 3 - метр, 4 - миллиметр, 5 - сантиметр.
...
def elif_r7(number, mass):
    ...
    Для уроков (составитель тестов) - килограмм, 2 - миллиграмм, 3 - грамм, 4 - тонна, 5 - центнер.
...
def elif_r12(element, number):
    ...
    Для уроков (составитель тестов) - радиус r, 2 - диаметр d = 2 * r, 3 - длина l = 2 * pi * r, 4 - площадь круга s = pi * r**2 .
...
def elif_r13(element, number):
    ...
    Для уроков (составитель тестов) - катет a, 2 - гипотенуза c = a * 2**0.5, 3 - высота h, опущенная на гипотенузу (h = c / 2),
...
def elif_r14(element, number):
    ...
    Для уроков (составитель тестов) - сторона a, 2 - радиус r1 вписанной окружности (r1 = a * 3**0.5 / 6),
...
def elif_r15(n, m):
    """
    Elif15. Мастям игральных карт присвоены порядковые номера: Для уроков (составитель тестов) - пики, 2 - трефы, 3 - бубны, 4 - червы.
    Достоинству карт, старших десятки, присвоены номера: 11 - валет, 12 - дама, 13 - король, 14 - туз.
    Даны два целых числа: num - достоинство (6 ≤ N ≤ 14) и m - масть карты (Для уроков (составитель тестов) ≤ M ≤ 4).
...
def elif_r18(int_number):
    """
    Elif18. Дано целое число в диапазоне Для уроков (составитель тестов)–999. Вывести строку-описание данного числа,
...
def elif_r20(day, month):
    ...
    Вывести знак Зодиака, соответствующий этой дате: «Водолей» (20.Для уроков (составитель тестов)–18.2), «Рыбы» (19.2–20.3), «Овен» (21.3–19.4),
    «Телец» (20.4–20.5), «Близнецы» (21.5–21.6), «Рак» (22.6–22.7), «Лев» (23.7–22.8), «Дева» (23.8–22.9),
    «Весы» (23.9–22.10), «Скорпион» (23.10–22.11), «Стрелец» (23.11–21.12), «Козерог» (22.12–19.Для уроков (составитель тестов))
    """
